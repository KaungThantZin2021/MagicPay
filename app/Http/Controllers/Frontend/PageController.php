...
class PageController extends Controller
{
    ...
    public function transferConfirm(TransferFormValidate $request){
        ...
        if ( $amount < 1000 ) {
            return back()->withErrors(['amount' => 'The amount must be at 1000 MMK.'])->withInput();
        }
        $to_account = User::where('phone' , $phone)->first();
        if (!$to_account) {
            return back()->withErrors(['to_phone' => 'This account does not exist.'])->withInput();
        }
    ...
    public function passwordCheck(Request $request){
        ...
        if (Hash::check($request->password , $authUser->password)) {
            return response()->json([
                'status' => 'success',
                'message' => 'The passowrd is correct.'
            ]);
        }
        return response()->json([
            'status' => 'fail',
    ...
    public function scanAndPayForm(Request $request){
        ...
        if (!$to_account) {
            return back()->withErrors(['fail' => 'This account does not exist.QR code is invalid.'])->withInput();
        }
